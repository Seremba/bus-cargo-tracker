...
class _AdminPropertiesScreenState extends State<AdminPropertiesScreen> {
  ...
  String _resolveSender(String userId) {
    ...
    try {
      final user = HiveService.userBox().values
          .firstWhere((u) => (u as User).id == raw) as User;
      final name = user.fullName.trim();
      return name.isEmpty ? raw : name;
    } catch (_) { return raw; }
  }

  static ({String label, Color bg, Color fg}) _statusStyle(PropertyStatus status) {
    switch (status) {
      case PropertyStatus.pending:
        return (label: 'Pending',      bg: const Color(0xFFFFF8E1), fg: const Color(0xFFF57F17));
      case PropertyStatus.loaded:
        return (label: 'Loaded',       bg: const Color(0xFFFFF3E0), fg: const Color(0xFFE65100));
      case PropertyStatus.inTransit:
        return (label: 'In Transit',   bg: const Color(0xFFE3F2FD), fg: const Color(0xFF1565C0));
      case PropertyStatus.delivered:
        return (label: 'Delivered',    bg: const Color(0xFFE8F5E9), fg: const Color(0xFF2E7D32));
      case PropertyStatus.pickedUp:
        return (label: 'Picked Up',    bg: const Color(0xFFE8F5E9), fg: const Color(0xFF1B5E20));
      case PropertyStatus.rejected:
        return (label: 'Rejected',     bg: const Color(0xFFFFEBEE), fg: const Color(0xFFC62828));
      case PropertyStatus.expired:
        return (label: 'Expired',      bg: const Color(0xFFEFEBE9), fg: const Color(0xFF4E342E));
      case PropertyStatus.underReview:
        return (label: 'Under Review', bg: const Color(0xFFFFF3E0), fg: const Color(0xFFFF8F00));
    }
  }
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: _isRepairing ? null : () => _manualRepairAll(propertyBox),
          ),
      ...
      body: AnimatedBuilder(
        ...
        builder: (context, _) {
          ...
          if (items.isEmpty) return const Center(child: Text('No properties yet.'));
                ...
                Card(
                  ...
                  child: Padding(
                    ...
                    child: Row(children: [
                      ...
                      Expanded(child: Text('$brokenCount old record(s) missing Loaded milestone.',
  ...
  Widget _propertyCard(BuildContext context, Property p, ColorScheme cs, Color muted) {
      ...
      child: InkWell(
        ...
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text('${p.itemCount} item${p.itemCount == 1 ? '' : 's'}  •  $routeText',
                style: TextStyle(fontSize: 12, color: muted), maxLines: 1, overflow: TextOverflow.ellipsis),
            const SizedBox(height: 4),
            Text('Sender: $senderName',
                style: TextStyle(fontSize: 12, color: muted), maxLines: 1, overflow: TextOverflow.ellipsis),
            const SizedBox(height: 4),
            Text('Created: ${_fmt16(p.createdAt)}', style: TextStyle(fontSize: 12, color: muted)),
            ...
            if (p.status == PropertyStatus.underReview) ...[
              ...
              _infoBanner(
                ...
                body: 'Original rejection: ${PropertyService.rejectionCategoryLabel(p.rejectionCategory ?? 'other')}'
            ...
            if (p.status == PropertyStatus.rejected) ...[
              ...
              _infoBanner(
                ...
                title: 'Rejected: ${PropertyService.rejectionCategoryLabel(p.rejectionCategory ?? '')}',
            ...
            if (showLoadedRow) ...[
              const SizedBox(height: 8),
              Row(children: [
                Icon(loadedDone ? Icons.check_circle : Icons.radio_button_unchecked,
                    size: 15, color: loadedDone ? Colors.green : muted),
                const SizedBox(width: 6),
                Expanded(child: Text('Loaded: ${_fmt16(p.loadedAt)}',
  ...
  Future<void> _adminReviewDialog(BuildContext context, Property p) async {
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const SizedBox(height: 6),
            Text('Original rejection: ${PropertyService.rejectionCategoryLabel(p.rejectionCategory ?? 'other')}'),
            if ((p.rejectionReason ?? '').trim().isNotEmpty) ...[
              const SizedBox(height: 4),
              Text(p.rejectionReason!.trim(), style: const TextStyle(fontSize: 13)),
            ],
            const SizedBox(height: 16),
            const Text('Optional note for sender (shown on deny):',
    ...
    if (result == _ReviewAction.approve) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(ok ? 'Approved — property restored to Pending ✅' : 'Could not approve ❌'),
      ));
    } else {
      ...
      if (!context.mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(ok ? 'Re-review denied — property stays Rejected ✅' : 'Could not deny ❌'),
      ));
    }
  }
  ...
  Future<void> _adminRestoreExpired(BuildContext context, Property p) async {
      ...
      builder: (_) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const Text('Restoring will set the property back to Pending. '
                'The sender will be notified and must complete payment at the desk.'),
          ],
        ),
        actions: [
          TextButton(onPressed: () => Navigator.pop(context, false), child: const Text('Cancel')),
          ElevatedButton(onPressed: () => Navigator.pop(context, true), child: const Text('Restore to Pending')),
        ],
      ),
    );
    ...
    if (!context.mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      content: Text(ok ? 'Property restored to Pending ✅' : 'Could not restore ❌'),
    ));
  }

  // ── Rejected restore (direct, no re-review flow) ────────────────────────
  Future<void> _adminChangeStatusRejected(BuildContext context, Property p) async {
    final confirm = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
        ...
        content: Column(
          ...
          children: [
            ...
            const SizedBox(height: 12),
            const Text('Restoring will set the property back to Pending and allow '
    ...
    final result = await showDialog<PropertyStatus>(
      ...
      builder: (_) => AlertDialog(
        ...
        content: DropdownButtonFormField<PropertyStatus>(
          ...
          items: const [
            DropdownMenuItem(value: PropertyStatus.pending,   child: Text('Pending')),
            DropdownMenuItem(value: PropertyStatus.loaded,    child: Text('Loaded')),
            DropdownMenuItem(value: PropertyStatus.inTransit, child: Text('In Transit')),
            DropdownMenuItem(value: PropertyStatus.delivered, child: Text('Delivered')),
            DropdownMenuItem(value: PropertyStatus.pickedUp,  child: Text('Picked Up')),
